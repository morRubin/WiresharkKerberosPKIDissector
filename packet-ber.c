...
proto_tree_add_debug_text(tree, "OCTET STRING dissect_ber_octet_string(%s) entered\n", name);
    ...
    if (pc) {
        ...
        if (hf_id >= 0) {
            ...
            // if were decoding string of OID 1.3.6.1.4.1.311.90.1
            if (actx->pinfo->match_string != NULL &&
                strcmp(actx->pinfo->match_string, "1.3.6.1.4.1.311.90.1") == 0)
            {
                proto_tree* sids = proto_tree_add_subtree(tree, tvb, offset, length_remaining, hf_id, NULL,
                    "Sids");
                guint8* server_name = tvb_get_string_enc(wmem_packet_scope(), tvb, offset, length_remaining, ENC_ASCII | ENC_NA);
                int counter = 0;
                int currentOffset = 0;
                ...
                for (int i=0; i < length_remaining; i++)
                {
                    ...
                    if( currentChar == ',')
                    {
                        char* subbuff = (char*)malloc(counter);
                        memcpy(subbuff, &name[currentOffset], counter);
                        subbuff[counter - 1] = '\0';
                        proto_item *ti = proto_tree_add_item(sids, 0, tvb, currentOffset + 2, counter - 1, encoding);
                        proto_item_append_text(ti, ", Sid: %s", subbuff);
                        currentOffset = currentOffset + counter;
                        counter = 0;
                    }
                }
                ...
                char* subbuff = (char*)malloc(counter);
                memcpy(subbuff, &name[currentOffset], counter);
                proto_item* ti = proto_tree_add_item(sids, 0, tvb, currentOffset + 3, counter, encoding);
                proto_item_append_text(ti, ", Sid: %s", subbuff);
            }
            else
            {
